...
import { isAuthenticated } from "./auth-system-new";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/plans-data", isAuthenticated, async (req: Request, res: Response) => {
    try {
      ...
      const planData = { ...req.body, userId: parseInt(userId) };
  ...
  app.post("/api/login", async (req, res) => {
    try {
      ...
      console.log("Login: Setting req.session.userId:", user.id);
...
Format as a brief analysis (2-3 sentences) focusing on what their data actually shows about their relationship patterns.`;
  ...
  app.get("/api/chat/conversations/:id", isAuthenticated, async (req, res) => {
    try {
      ...
      console.log(`Authorization check for conversation ${conversationId}: conversation.userId="${conversationUserIdStr}", req.session.userId="${sessionUserIdStr}"`);
  ...
  app.delete("/api/chat/conversations/:id", isAuthenticated, async (req, res) => {
    try {
      ...
      console.log(`Authorization check: conversation.userId="${conversationUserIdStr}", req.session.userId="${sessionUserIdStr}"`);
  ...
  app.post("/api/support/send", isAuthenticated, async (req: Request, res: Response) => {
    ...
    console.log('🔔 SUPPORT - Session userId:', req.session?.userId);
